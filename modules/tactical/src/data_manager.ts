...
export class TacticalDataManager implements DataManager {
  ...
  constructor(backend: Backend) {
    ...
    var that = this;
    backend.data().subscribe((data) => { that._backendData(data); });
  }

  /**
   * Called when data arrives from the backend.
   */
  _backendData(data: VersionedObject) {
    var keyStr = serializeValue(data.key);
    if (!this._active.hasOwnProperty(keyStr)) {
      return;
    }
    this._active[keyStr].forEach((link) => { link.send(data.data); });
  }
   ...
   */
  _requestFromStore(key: Object, link: Link<Object>) {
