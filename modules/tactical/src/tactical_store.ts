...
export class TacticalStore {
  ...
  private static _versionStore: string = "tac_versions_";
  private static _recordStore: string = "tac_records_";
  
  constructor(private _idbRef: Idb, public store = '') {}
  
  /**
   * Used to read a Record from persistent local cache. Emits the Record that
  ...
  fetch(key: Object, version?: string): Observable<Record> {
    ...
    return Observable.create<Record>((observer: Rx.Observer<Record>) => {
      if (version) {
        ...
        this._idbRef.get(TacticalStore._recordStore + this.store, recordKey.serial).subscribe(
          (value: Object) => {
            if (value) {
        ...
        this._idbRef.get(TacticalStore._versionStore + this.store, chainKey.serial).subscribe(
          (ver: Version) => {
            if (ver && ver.version) {
  ...
  /**
   * Used to store a Record into persistent local cache. This will set the most recent version
   * to be the version provided and overwrite the most recent Record, if it is currently unanchored.
   * TODO(iri): overwrite unanchored Records.
   */
  commit(key: Object, value: Object, version: string): Observable<boolean> {
    return Observable.create<boolean>((observer: Rx.Observer<boolean>) => {
      ...
      this._idbRef.put(TacticalStore._recordStore + this.store, recordKey.serial, value).subscribe(
        (ok: boolean) => {
          if (ok) {
            var ver: Version = {version: version};
            // update the most recent version in the record chain
            this._idbRef.put(TacticalStore._versionStore + this.store, recordKey.chain.serial, ver)
                .subscribe(
              (ok: boolean) => {
                observer.onNext(ok);
              },
