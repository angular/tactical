...
describe("Tactical Store", () => {
  ...
  it("should store new Records", (done) => {
    ...
    tacStore.commit(fooKey.chain.key, fooValue, fooKey.version).subscribe(
      (ok: boolean) => {
        if (ok) {
          mockIdb.get("_records_tactical_", fooKey.serial).subscribe(
            (value: Object) => {
              ...
              if (ok) {
                mockIdb.get("_records_tactical_", fooKey.serial).subscribe(
                  (value: Object) => {
                    ...
                    mockIdb.get("_records_tactical_", barKey.serial).subscribe(
                      (otherValue: Object) => {
  ...
  it("should use the store provided", (done) => {
    ...
    tacStore.commit(fooKey.chain.key, fooValue, fooKey.version).subscribe(
      (ok: boolean) => {
        if (ok) {
          mockIdb.get("test_records_tactical_", fooKey.serial).subscribe(
            (value: Object) => {
              expect(value['foo']).to.equal(fooValue['foo']);
              done();
            },
