...
import {SetPropertiesMutation, SubPropertyMutation, ArrayValueMutation,
    ArrayTruncationMutation, ArraySubMutation} from '../src/mutation';
...
describe('SubPropertyMutation', () => {
  it('applies child mutations', () => {
    ...
    var children = [
      new SetPropertiesMutation({'version': 2}),
      new SetPropertiesMutation({'other': 3})
    ];
  ...
  it('fails when the child key does not exist', () => {
    var obj = {
      'foo': 42
    };
    var children = [
      new SetPropertiesMutation({'shouldFail': true})
    ];
...
describe('ArrayTruncationMutation', () => {
  ...
  it('fails when the array is too short already', () => {
    var obj = [1];
    var mutation = new ArrayTruncationMutation(3);
    expect(mutation.apply(obj)).to.be.false;
  });
...
describe('ArraySubMutation', () => {
  it('applies mutations to a value in the array', () => {
    var obj = [{}, {}];
    var children = [
      new SetPropertiesMutation({'foo': true})
    ];
  ...
  it('fails when the index does not exist in the target array', () => {
    var obj = [{}, {}];
